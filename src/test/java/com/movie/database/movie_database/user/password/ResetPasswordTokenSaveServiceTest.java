...
class ResetPasswordTokenSaveServiceTest {
    ...
    @Test
    @DisplayName("Should update reset password token when already exists")
    public void shouldUpdateResetPasswordTokenWhenAlreadyExists() {
        //arrange
        ...
        var applicationUserToken = new ApplicationUserToken(user, oldToken, TokenType.RESET_PASSWORD);
        when(applicationUserGetService.findById(userId)).thenReturn(user);
        when(applicationUserTokenRepository.findByApplicationUserAndTokenType(user, TokenType.RESET_PASSWORD)).thenReturn(Optional.of(applicationUserToken));
    ...
    @Test
    @DisplayName("Should create new reset password token when not exists")
    public void shouldCreateNewResetPasswordTokenWhenNotExist() {
        //arrange
        var userId = UUID.fromString("78688d94-86ea-11e9-bc42-526af7764f64");
        var token = JWT.create().withSubject(userId.toString()).withExpiresAt(Date.from(Instant.now().plusSeconds(30))).sign(Algorithm.HMAC256("secret"));
        var user = new ApplicationUser("username", "password", "email@email.ca");
        ...
        when(applicationUserGetService.findById(userId)).thenReturn(user);
        when(applicationUserTokenRepository.findByApplicationUserAndTokenType(user, TokenType.RESET_PASSWORD)).thenReturn(Optional.empty());
